...
**`src/data/wfh.json`** - WFH安排数据
```json
[
  {"name": "Alex Yang", "weekday": "thu"},
  {"name": "Alex Yang", "weekday": "fri"}
]
```

**`src/data/leave.json`** - 请假记录数据
```json
[
  {"name": "Dave Meng", "leaveDate": "2026-01-19"}
]
```

#### 数据服务

通过 `src/services/recordService.ts` 提供统一的数据访问接口：
- `WFHData.getAll()` - 获取所有WFH记录
- `WFHData.getWFHToday(weekday)` - 获取今日WFH人员
- `LeaveData.getToday(date)` - 获取今日请假人员
- `LeaveData.getNextWeek()` - 获取未来一周请假记录
...
邮件生成器，支持四种邮件模板：
1. **启动加班** - OT Record Initiation
2. **结束加班** - OT Completed（仅HTML预览，无创建按钮）
3. **申请WFH** - WFH Request
4. **申请假期** - Leave Request

#### 功能特点

- **表单验证**：使用 react-hook-form + Zod 进行验证
- **自动填充**：发件人姓名自动从 localStorage 读取
- **智能计算**：结束加班表单自动计算加班时长（支持跨天）
- **双模式输出**：
  - **创建邮件**：打开邮件客户端（纯文本，可自动添加签名）
  - **HTML预览**：带格式预览 + 一键复制（保留格式）
- **自动预览**：表单填写完整后自动展开HTML预览
...
- `[X hours]` - 时长
- `[TASK]`、`[REASON]`、`[REMARK]` - 文本说明
- `[ID]` - JIRA/SR/CR编号
- `[COMPENSATION]` - 补偿类型
- `[BACKUP]`、`[CONTACT]` - 请假相关
- `[WFH-TYPE]`、`[WFH-DATES]` - WFH相关
